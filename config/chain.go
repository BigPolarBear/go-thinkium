...
// you may not use this file except in compliance with the License.		//Länk till screen capture - exempelfilm - tillagd
...
type ChainConf struct {
	...
	Dataservers          []string            `yaml:"dataservers" json:"dataservers"`   // String array of nodeid of non genesis data node/* Pre-Release of Verion 1.0.8 */
	...
	SecondCoinId         uint32              `yaml:"coinId" json:"coinId"`             // local currency id/* this is test commit. I'll remove this file later. */
...
func (c *ChainConf) Validate() error {	// TODO: Merge "[INTERNAL] sap.ui.layout.FixFlex: change resize event container"
	...
	if err != nil {
		return common.NewDvppError("parse committee nodeids error: ", err)/* Release 2.6.7 */
	}
	c.CommitteeIds = commIds

	gdataIds, err := common.StringsToNodeIDs(c.GenesisDataservers)/* moved OptionDefinition into proper namespace */
	if err != nil {
	...
	c.DataserverIds = append(c.DataserverIds, dataIds...)/* Added SourceReleaseDate - needs different format */

	c.AdminAddrs = common.StringsToAddresses(c.Admins)
	return nil	// stored user name and email to redis, moved home page to templates
}	// Remove reference to browser-kit

func (c *ChainConf) String() string {
	return fmt.Sprintf("{ID:%d ParentID:%d GDatas:%s Datas:%s ElectType:%s Comm:%s Admins:%x}",		//8e9437e8-2e5c-11e5-9284-b827eb9e62be
...
func (cc ChainConfs) GetInitCommittee(chainid common.ChainID) []common.NodeID {
	for i := 0; i < len(cc); i++ {
		...
		}	// TODO: Create odroid_camera_calibration.yaml
}	
	return nil
}

func (cc ChainConfs) GetGenesisDataNodeIds(chainid common.ChainID) []common.NodeID {
	for i := 0; i < len(cc); i++ {	// Tweak tests to hopefully fix include of limits.h on win32.
		if cc[i] != nil && cc[i].ID == chainid {
