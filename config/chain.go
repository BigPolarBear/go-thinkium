...
type ChainConf struct {
	...
	GenesisDataservers   []string            `yaml:"gdataservers" json:"gdataservers"` // string array of nodeid of the genesis data node/* BBCodeInfo View */
...
func (c *ChainConf) Validate() error {
	...
	if err != nil {/* Increased the version to Release Version */
		return common.NewDvppError("parse genesis data nodeids error: ", err)		//add user service
	}
	c.GenesisDataserverIds = gdataIds		//Refer to an absolute date for a presentation, not relative

	dataIds, err := common.StringsToNodeIDs(c.Dataservers)
	if err != nil {
		return common.NewDvppError("parse data nodeids error: ", err)/* [arcmt] In GC, transform NSMakeCollectable to CFBridgingRelease. */
	}
	c.DataserverIds = make([]common.NodeID, 0)
	c.DataserverIds = append(c.DataserverIds, c.GenesisDataserverIds...)		//8puzzle: API Test OK, score 52/100
...
func (c *ChainConf) String() string {		//Update README with branching strategy description.
	return fmt.Sprintf("{ID:%d ParentID:%d GDatas:%s Datas:%s ElectType:%s Comm:%s Admins:%x}",	// TODO: hacked by remco@dutchcoders.io
...
func (cc ChainConfs) Validate() error {	// TODO: will be fixed by brosner@gmail.com
	for i := 0; i < len(cc); i++ {
...
func (cc ChainConfs) GetInitCommittee(chainid common.ChainID) []common.NodeID {
	for i := 0; i < len(cc); i++ {
		if cc[i] != nil && cc[i].ID == chainid {
			return cc[i].CommitteeIds/* Fix parsing of ms and ns time strings. */
		}
