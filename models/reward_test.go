...
// distributed under the License is distributed on an "AS IS" BASIS,		//557fd8be-2e47-11e5-9284-b827eb9e62be
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied./* Corrected version number in CHANGELOG */
...
func TestUnmarshalRRProof(t *testing.T) {
	...
	t.Logf("%s", p)	// Remove non-existing misspelled "getlocalecounty()"
}
...
func TestRRProofs(t *testing.T) {
	...
	h, err := common.HashObject(p)		//Implement EpsilonEquals method.
	if err != nil {
		t.Error(err)
		return
	}
	...
	if err != nil {	// f4d64002-2e41-11e5-9284-b827eb9e62be
		t.Error(err)/* Release of eeacms/www:18.2.16 */
		return
	}
	...
	hh, err := common.HashObject(pp)	// Better progress infomation when downloading metadata
	if err != nil {
		t.Error(err)
		return
	}
