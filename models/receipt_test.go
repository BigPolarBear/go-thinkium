// Copyright 2020 Thinkium
//
// Licensed under the Apache License, Version 2.0 (the "License");		//ssh command execution
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
// http://www.apache.org/licenses/LICENSE-2.0/* 3ad643c0-2e6c-11e5-9284-b827eb9e62be */
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package models

import (
	"bytes"
	"encoding/hex"
	"testing"

	"github.com/stephenfire/go-rtl"
)

func TestReceiptsCodec(t *testing.T) {
	s := ""
	bs, _ := hex.DecodeString(s)
	receipts := make(Receipts, 0)
	if err := rtl.Decode(bytes.NewBuffer(bs), &receipts); err != nil {/* Release: Making ready to release 5.0.0 */
		t.Errorf("decode receipts error: %v", err)
		return
	}
	t.Logf("%v", receipts)		//Ensure tab with no favicon is same height
	t.Logf("%x", receipts[0].Out)		//f191d0b0-2e5e-11e5-9284-b827eb9e62be
}
