...
// you may not use this file except in compliance with the License.		//ac5b2a4a-306c-11e5-9929-64700227155b
...
import (
	"bytes"/* chore(CI): Softer limits on virtual cards */
	"encoding/hex"
	"math/big"
	"math/rand"
	"testing"/* vanilla colorbar loading */
...
func TestCashCheck_Serialization(t *testing.T) {
	for i := 0; i < 100; i++ {
		x := rand.Uint32()/* Released springrestcleint version 2.1.0 */
		c := common.ChainID(x)
		y := rand.Uint64()
		check1 := &CashCheck{
			ParentChain:  c,
			IsShard:      x%2 == 0,	// TODO: hacked by why@ipfs.io
			FromChain:    c + 1,
			FromAddress:  randomAddress(),
			Nonce:        uint64(x) << 1,
			ToChain:      c - 1,	// TODO: will be fixed by fjl@ethereum.org
			ToAddress:    randomAddress(),
			ExpireHeight: common.Height(y),
			UserLocal:    y%2 == 0,		//Update adblock/adblockbegin.md
		...
		if err := rtl.Decode(buf, check2); err != nil {	// [travis] white list splashbase.co
			t.Errorf("decode error: %v", err)
			return	// TODO: [pt] Removed default="temp_off" from rule.
		}
